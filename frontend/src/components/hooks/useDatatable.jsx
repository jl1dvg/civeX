...
const useDataTable = ({
                          ...
                          isFooter = false,
                          isMultilang = false,
                          rowCallback, // <--- NUEVO: prop que nos pasará la función callback
                          order = [], // Agregamos order como parámetro opcional
                      }) => {
    useEffect(() => {
        setTimeout(() => {
            let datatableObj = {
                ...
                columns: columns,
                destroy: true,
                order: order, // Incluimos el orden personalizado aquí
            };

            if (url) {
                datatableObj = {
                    ...datatableObj, processing: true, serverSide: true, ajax: {
                        url: url,
                    },
                };
            }
            ...
            if (typeof rowCallback === "function") {
                datatableObj = {
                    ...datatableObj, rowCallback: function (row, rowData, displayIndex) {
                        // DataTables llama esto para cada <tr>
                        // Reenviamos a la función que definiremos en ProtocolosList
                        rowCallback(row, rowData, displayIndex);
                    }
                };
            }
            ...
            if (typeof actionCallback === "function") {
                $(datatable.table().body()).on("click", '[data-table="action"]', function () {
                    actionCallback({
                        id: $(this).data("id"), method: $(this).data("method"),
                    });
                });
            }
    ...
    }, [tableRef, columns, data, url, actionCallback, isColumnHidden, isColumnHiddenClass, isFilterColumn, isFooter, isMultilang, rowCallback,    // <--- Añadirlo como dependencia
        order, // Incluimos order como dependencia
    ]);
};
